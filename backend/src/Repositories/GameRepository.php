...
final class GameRepository
{
    ...
    public function loadGame(AuthUser $user): array
    {
        ...
        return [
            'user' => $user->toArray(),
            'game_state' => $save === null ? null : $this->decodeState((string) $save['game_state_json']),
    ...
    private function persistState(string $authUserId, array $gameState): void
    {
        ...
        $stmt = $this->db->prepare(
            'INSERT INTO game_saves (auth_user_id, game_state_json, created_at, updated_at)
             VALUES (:auth_user_id, :game_state_json, :created_at, :updated_at)
             ON DUPLICATE KEY UPDATE
    ...
    public function moveGuestSaveToUser(string $guestUserId, AuthUser $targetUser): array
    {
        ...
        try {
            ...
            $move = $this->db->prepare(
                'UPDATE game_saves SET auth_user_id = :target_user_id, updated_at = :updated_at WHERE id = :save_id'
            );
            $move->execute([
    ...
    {
        $stmt = $this->db->prepare('SELECT * FROM game_saves WHERE auth_user_id = :auth_user_id LIMIT 1');
