...
async function testBalance() {
  try {
    ...
    console.log("========================");
    console.log(
      `Win Rate: ${((result.playerWins / result.totalGames) * 100).toFixed(1)}%`,
    );
    console.log(
      `Games: ${result.playerWins}W-${result.enemyWins}L-${result.draws}D`,
    );
    ...
    console.log("\n🎯 Balance Fix Status:");
    console.log(
      `  - Games lasting 15+ turns: ${result.gameLengths.filter((l) => l >= 15).length}/${result.totalGames}`,
    );
    console.log(
      `  - Win rate in balanced range (30-70%): ${result.playerWins / result.totalGames >= 0.3 && result.playerWins / result.totalGames <= 0.7 ? "✅ Yes" : "❌ No"}`,
    );
  } catch (error) {
