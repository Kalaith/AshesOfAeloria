...
export class StoryEventSystem {
    ...
    for (const consequence of consequences) {
      switch (consequence.type) {
        ...
        case 'unlock_technology':
          if (consequence.target && !this.gameState.globalTechnologies.includes(consequence.target as any)) {
  ...
  private applyCorruptionChange(consequence: EventConsequence): void {
    if (consequence.target === 'global') {
      this.gameState.worldState.corruptionLevel = Math.max(0,
  ...
  private applyPopulationChange(consequence: EventConsequence): void {
    ...
    if (playerNodes.length > 0) {
      ...
      if (target && target in largestNode.population) {
        (largestNode.population as any)[target] = Math.max(0,
   ...
   */
  public getEventHistory(): Array<{eventId: string, choiceId: string, turn: number, description: string}> {
