...
export class AIPlayer {
  ...
  analyzeGameState(gameState: GameState): AIDecision[] {
    ...
    const militaryStrength = this.calculateMilitaryStrength(playerNodes, playerCommanders);
    const enemyStrength = this.calculateMilitaryStrength(enemyNodes, enemyCommanders);
    ...
    const defensiveDecisions = this.analyzeDefensiveNeeds(gameState, playerNodes, enemyNodes);
    decisions.push(...defensiveDecisions);

    // Priority 2: Economic development if safe
    if (militaryStrength >= enemyStrength * 0.8) {
      const economicDecisions = this.analyzeEconomicOpportunities(gameState, playerNodes);
      decisions.push(...economicDecisions);
    }

    // Priority 3: Military expansion opportunities
    const expansionDecisions = this.analyzeExpansionOpportunities(gameState, playerNodes, neutralNodes, enemyNodes);
    decisions.push(...expansionDecisions);

    // Priority 4: Commander management
    const commanderDecisions = this.analyzeCommanderNeeds(gameState, playerNodes, playerCommanders);
   ...
   */
  private calculateMilitaryStrength(nodes: GameNode[], commanders: Commander[]): number {
    let strength = 0;

    // Add garrison strength
    nodes.forEach(node => {
      const nodeCommanders = commanders.filter(c => c.assignedNode === node.id);
      const effectiveGarrison = calculateEffectiveGarrison(node, nodeCommanders);
   ...
   */
  private analyzeDefensiveNeeds(gameState: GameState, playerNodes: GameNode[], enemyNodes: GameNode[]): AIDecision[] {
    const decisions: AIDecision[] = [];
    ...
    playerNodes.forEach(playerNode => {
      ...
      threateningEnemies.forEach(enemy => {
        const enemyCommanders = gameState.commanders.filter(c =>
          c.owner === 'enemy' && c.assignedNode === enemy.id
        );
        const playerCommanders = gameState.commanders.filter(c =>
          c.owner === 'player' && c.assignedNode === playerNode.id
        );

        const enemyStrength = calculateEffectiveGarrison(enemy, enemyCommanders).totalPower;
        const playerStrength = calculateEffectiveGarrison(playerNode, playerCommanders).totalPower;

        if (enemyStrength > playerStrength * 1.2) { // Threat detected
          // Check if we can reinforce this node
          const availableCommanders = gameState.commanders.filter(c =>
            c.owner === 'player' && !c.assignedNode
          );
   ...
   */
  private analyzeEconomicOpportunities(gameState: GameState, playerNodes: GameNode[]): AIDecision[] {
    const decisions: AIDecision[] = [];
   ...
   */
  private analyzeExpansionOpportunities(gameState: GameState, playerNodes: GameNode[], neutralNodes: GameNode[], enemyNodes: GameNode[]): AIDecision[] {
    const decisions: AIDecision[] = [];

    // Find attackable neutral and enemy nodes
    const targets: Array<{node: GameNode, difficulty: number, value: number}> = [];
    ...
    playerNodes.forEach(playerNode => {
      ...
      playerNode.connections.forEach(connectionId => {
        const targetNode = [...neutralNodes, ...enemyNodes].find(n => n.id === connectionId);
        if (targetNode) {
          const targetCommanders = gameState.commanders.filter(c =>
            c.owner === targetNode.owner && c.assignedNode === targetNode.id
          );
          const targetStrength = calculateEffectiveGarrison(targetNode, targetCommanders).totalPower;

          const winChance = this.calculateWinChance(playerStrength, targetStrength);
          const nodeValue = this.calculateNodeValue(targetNode);

          if (winChance > 0.6) { // Only consider if decent chance of success
            targets.push({
              node: targetNode,
    ...
    targets.sort((a, b) => (b.value / (b.difficulty + 0.1)) - (a.value / (a.difficulty + 0.1)));
   ...
   */
  private analyzeCommanderNeeds(gameState: GameState, playerNodes: GameNode[], playerCommanders: Commander[]): AIDecision[] {
    const decisions: AIDecision[] = [];
    ...
    const unassignedCommanders = playerCommanders.filter(c => !c.assignedNode);
    unassignedCommanders.forEach(commander => {
      const bestNode = this.findBestNodeForCommander(commander, playerNodes, playerCommanders);
      if (bestNode) {
        decisions.push({
          type: 'assign',
          target: bestNode.id,
   ...
   */
  private calculateWinChance(attackerStrength: number, defenderStrength: number): number {
   ...
   */
  private findBestNodeForCommander(commander: Commander, playerNodes: GameNode[], allCommanders: Commander[]): GameNode | null {
  ...
  getAnalysisSummary(): any {
    return {
      ...
      decisionTypes: this.decisions.reduce((acc, d) => {
