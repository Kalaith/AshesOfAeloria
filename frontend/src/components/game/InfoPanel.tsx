...
export const InfoPanel: React.FC = () => {
  ...
  const assignCommanderToNode = useGameStore(
    (state) => state.assignCommanderToNode,
  );
  const unassignCommander = useGameStore((state) => state.unassignCommander);
  const getNodeCommanderInfo = useGameStore(
    (state) => state.getNodeCommanderInfo,
  );
  ...
  const commander =
    selectedCommander !== null
      ? commanders.find((c) => c.id === selectedCommander)
      : null;
  ...
  return (
    <Card className="p-3 lg:p-4">
      <h3 className="text-base lg:text-lg font-bold mb-3 lg:mb-4 text-gray-800">
        Information
      </h3>
        ...
        {node ? (
          <div className="space-y-3 lg:space-y-4">
            <div className="flex items-center gap-2 lg:gap-3 p-2 lg:p-3 bg-gray-50 rounded-lg">
              <span className="text-xl lg:text-2xl">
                {gameData.nodeTypes[node.type].icon}
              </span>
            ...
            <div className="grid grid-cols-1 gap-2">
              ...
              <div className="flex justify-between items-center py-1">
                ...
                <span className="text-xs lg:text-sm font-medium text-yellow-600">
                  {gameData.nodeTypes[node.type].goldGeneration}
                </span>
              </div>
              <div className="flex justify-between items-center py-1">
                <span className="text-xs lg:text-sm text-gray-600">
                ...
                <span className="text-xs lg:text-sm font-medium text-green-600">
                  {gameData.nodeTypes[node.type].suppliesGeneration}
                </span>
              </div>
              <div className="flex justify-between items-center py-1">
                <span className="text-xs lg:text-sm text-gray-600">
                ...
                <span className="text-xs lg:text-sm font-medium text-purple-600">
                  {gameData.nodeTypes[node.type].manaGeneration}
                </span>
              </div>
            </div>
            <p className="text-xs lg:text-sm text-gray-500 bg-gray-50 p-2 lg:p-3 rounded-lg">
              {gameData.nodeTypes[node.type].description}
            </p>
            ...
            {node.owner === "player" && (
              <div className="space-y-2 lg:space-y-3 pt-2 border-t border-gray-200">
                <div className="flex justify-between items-center">
                  <h4 className="text-sm lg:text-md font-semibold text-gray-800">
                    Commanders:
                  </h4>
                    ...
                    <div className="space-y-2">
                            ...
                            >
                              ...
                              <div className="flex-1">
                                <div className="text-sm font-medium text-green-800">
                                  {assignedCommander.name}
                                </div>
                                <div className="text-xs text-green-600">
                                  Level {assignedCommander.level} • Stationed
                                  here
                                </div>
                              </div>
                              <Button
                                ...
                                onClick={() =>
                                  handleUnassignCommander(assignedCommander.id)
                                }
                              >
                                🔄
                              </Button>
                        ...
                        availableCommanders.length > 0 && (
                          <div className="space-y-2">
                            <div className="text-xs text-gray-600 mb-2">
                              Assign commander to this{" "}
                              {gameData.nodeTypes[node.type].name.toLowerCase()}
                              :
                            </div>
                            {availableCommanders
                              .slice(0, 3)
                              .map((availableCommander) => (
                                <Button
                                  ...
                                  onClick={() =>
                                    handleAssignCommander(
                                      availableCommander.id,
                                      node.id,
                                    )
                                  }
                                >
                                  <span className="mr-2">
                                    {availableCommander.class === "knight" &&
                                      "⚔️"}
                      ...
                      {commanderInfo.current >= commanderInfo.max && (
                        <div className="text-xs text-gray-500 italic bg-gray-50 p-2 rounded">
                          {gameData.nodeTypes[node.type].name} is at maximum
                          commander capacity
                        </div>
                      )}
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center py-2 px-3 bg-gray-50 rounded">
                    ...
                    <span className="text-sm font-medium">
                      {"⭐".repeat(node.starLevel || 1)} ({node.starLevel || 1}
                      /5)
                    </span>
                  </div>
                ...
                <div className="space-y-2 lg:space-y-3 pt-2 border-t border-gray-200">
                  ...
                  <div className="flex flex-col gap-2">
                    {getAttackableNodes().map((targetNode) => (
                      ...
                      >
                        <span className="mr-2">
                          {gameData.nodeTypes[targetNode.type].icon}
                        </span>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-2">
              <div className="flex justify-between items-center py-1">
                ...
                <span className="text-sm font-medium">
                  {gameData.commanderClasses[commander.class].name}
                </span>
              </div>
              <div className="flex justify-between items-center py-1">
                <span className="text-sm text-gray-600">Race:</span>
                <span className="text-sm font-medium">
                  {gameData.races[commander.race].name}
                </span>
              </div>
              <div className="flex justify-between items-center py-1">
            ...
            <div className="space-y-2">
              <p className="text-sm text-gray-500 bg-gray-50 p-3 rounded-lg">
                {gameData.commanderClasses[commander.class].description}
              </p>
              <p className="text-sm text-gray-500 bg-blue-50 p-3 rounded-lg">
                <span className="font-medium text-blue-800">Special:</span>{" "}
                {gameData.commanderClasses[commander.class].specialAbility}
              </p>
            </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="flex items-center gap-2 p-2 bg-gray-50 rounded">
                  ...
                  <span className="text-sm">
                    Soldiers: {commander.army.soldiers}
                  </span>
                </div>
                <div className="flex items-center gap-2 p-2 bg-gray-50 rounded">
                  <span>🏹</span>
                  <span className="text-sm">
                    Archers: {commander.army.archers}
                  </span>
                </div>
                <div className="flex items-center gap-2 p-2 bg-gray-50 rounded">
                  <span>🐎</span>
                  <span className="text-sm">
                    Cavalry: {commander.army.cavalry}
                  </span>
                </div>
                <div className="flex items-center gap-2 p-2 bg-gray-50 rounded">
                  <span>🔥</span>
          ...
          <div className="text-center py-8">
            <p className="text-gray-500">
              Select a node or commander for details
            </p>
          </div>
        )}
      </div>
