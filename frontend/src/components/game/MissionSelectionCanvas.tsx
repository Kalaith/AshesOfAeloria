...
export const MissionSelectionCanvas: React.FC<MissionSelectionCanvasProps> = ({
  ...
  const { isChapterUnlocked, isChapterCompleted, isChapterActive, getChapterProgress } =
  ...
  return (
    <div className="h-full flex flex-col bg-stone-texture p-4">
      {/* Header */}
      <div className="mb-6">
        <h2 className="text-2xl font-frontier font-bold text-on-dark mb-2">🗺️ Campaign Missions</h2>
                    ...
                    <div className="space-y-1">
                      {selectedChapter.specialRules.map(rule => (
                        <div key={rule.id} className="text-sm text-crystal font-parchment">
                          <span className="font-frontier font-bold">{rule.name}:</span>{' '}
                          {rule.description}
                        </div>
                      ))}
                    </div>
