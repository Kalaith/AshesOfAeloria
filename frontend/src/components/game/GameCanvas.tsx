...
export const GameCanvas: React.FC = () => {
      ...
      nodes.forEach(node => {
        ...
        if (commanderInfo.current > 0) {
          ...
          const commanderColor =
            node.owner === 'player' ? '#FFD700' : node.owner === 'enemy' ? '#FF4444' : '#CCCCCC';
          ...
          if (commanderInfo.current > 6) {
            ctx.fillStyle =
              node.owner === 'enemy' ? 'rgba(255, 100, 100, 0.8)' : 'rgba(255, 0, 0, 0.8)';
            ctx.beginPath();
  ...
  const handleCanvasClick = useCallback(
    (event: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      const offsetX = (canvas.width - (maxX - minX) * calculatedScale) / 2 - minX * calculatedScale;
      const offsetY =
        (canvas.height - (maxY - minY) * calculatedScale) / 2 - minY * calculatedScale;
