...
export const GameCanvas: React.FC = () => {
      ...
      for (let col = 0; col < gridCols; col++) {
        for (let row = 0; row < gridRows; row++) {
          ...
          for (let blade = 0; blade < grassBlades; blade++) {
            const bladeX =
              x + 5 + ((col * 17 + row * 13 + blade * 7) % (gridSize - 10));
            const bladeY =
              y + 5 + ((row * 19 + col * 11 + blade * 3) % (gridSize - 10));
          ...
          if ((col + row * 3) % 7 === 0) {
            ...
            const flowerX =
              x + gridSize * 0.3 + ((col * 23) % (gridSize * 0.4));
            const flowerY =
              y + gridSize * 0.3 + ((row * 29) % (gridSize * 0.4));
            ctx.beginPath();
      ...
      if (selectedNode) {
        ...
        if (selectedNodeData) {
          ...
          const gridX = Math.round(
            (selectedNodeData.x - gridOffsetX - gridSize / 2) / gridSize,
          );
          const gridY = Math.round(
            (selectedNodeData.y - gridOffsetY - gridSize / 2) / gridSize,
          );
  ...
  const render = useCallback(() => {
    ...
    // Center the map
    const offsetX =
      (canvas.width - (maxX - minX) * calculatedScale) / 2 -
      minX * calculatedScale;
    const offsetY =
      (canvas.height - (maxY - minY) * calculatedScale) / 2 -
      minY * calculatedScale;
