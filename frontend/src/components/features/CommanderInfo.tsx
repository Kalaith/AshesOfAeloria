...
const StatItem: React.FC<StatItemProps> = ({ label, value, icon, color = 'text-iron-dark' }) => (
...
const ArmyComposition: React.FC<{ army: Commander['army'] }> = ({ army }) => (
  <div className="space-y-1">
    <div className="text-sm font-frontier font-bold text-iron-dark mb-2 text-battle-worn">⚔ War Band:</div>
    <StatItem label="Infantry" value={army.soldiers} icon="⚔" color="text-blood" />
    <StatItem label="Archers" value={army.archers} icon="🏹" color="text-forest" />
    <StatItem label="Cavalry" value={army.cavalry} icon="🐎" color="text-amber" />
    <StatItem label="War Mages" value={army.mages} icon="🔮" color="text-mana" />
    <div className="text-xs text-bronze mt-2 font-parchment">
      Total Forces: {army.soldiers + army.archers + army.cavalry + army.mages} warriors
    </div>
  </div>
);
...
export const CommanderInfo: React.FC<CommanderInfoProps> = ({
  ...
  return (
    <Card className={`p-4 ${className}`}>
      <h3 className="text-lg font-frontier font-bold mb-4 text-iron-dark text-battle-worn">⚔ War Leader Profile</h3>
      ...
      <div className="flex items-center gap-3 p-3 bg-bronze-texture rounded-lg mb-4 border-2 border-bronze">
        ...
        <div className="flex-1">
          <div className="font-frontier font-bold text-parchment-light">{commander.name}</div>
          <div className="text-xs text-parchment font-parchment">
            Level {commander.level} {race.name} {commanderClass.name}
          </div>
        </div>
        <div className="text-right">
          <div className={`text-xs px-3 py-1 rounded font-frontier font-bold border-2 ${
            isAssigned ? 'bg-forest/20 text-forest border-forest' : 'bg-iron/20 text-iron border-iron'
          }`}>
      ...
      <div className="mb-4">
        <div className="flex justify-between text-sm mb-1">
          <span className="text-parchment-dark font-parchment">🩸 Battle Wounds</span>
          <span className="font-frontier font-bold text-iron-dark">{commander.health}/{commander.maxHealth}</span>
        </div>
        <div className="w-full bg-iron-dark rounded-full h-3 border border-bronze">
          <div
            className={`h-3 rounded-full transition-all duration-500 ${
              healthPercentage > 60 ? 'bg-forest animate-forge-flicker' :
              healthPercentage > 30 ? 'bg-amber animate-ember-glow' : 'bg-blood animate-battle-shake'
            }`}
      ...
      <div className="space-y-1 mb-4">
        <StatItem label="Battle Prowess" value={commander.attack} icon="⚔" color="text-blood" />
        <StatItem label="Tactical Defense" value={commander.defense} icon="🛡" color="text-mana" />
        <StatItem label="War Experience" value={commander.experience} icon="⭐" color="text-amber" />
      </div>
