...
export const useGameplayTesting = () => {
  ...
  const runTest = useCallback(
    async (strategy: string = "balanced", iterations: number = 25) => {
      setState((prev) => ({
        ...prev,
        isRunning: true,
      ...
      try {
        console.log(
          `🚀 Starting gameplay test: ${strategy} strategy, ${iterations} iterations`,
        );
        ...
        console.log(
          `⏱️ Average game length: ${result.averageTurns.toFixed(1)} turns`,
        );

        if (result.commonBalanceIssues.length > 0) {
          console.log(`⚠️ Balance issues detected:`);
          result.commonBalanceIssues.forEach((issue) =>
            console.log(`  - ${issue}`),
          );
        }
  ...
  const runAllStrategies = useCallback(
    async (iterations: number = 10) => {
      const strategies = Object.keys(aiStrategies);
      const results: BalanceReport[] = [];

      console.log(
        `🤖 Running comprehensive balance test across ${strategies.length} strategies...`,
      );
      ...
      if (results.length > 0) {
        ...
        results.forEach((result) => {
          const strategy =
            result.detailedResults[0]?.strategy.name || "Unknown";
          const winRate = result.playerWins / result.totalGames;
          const rating = getBalanceRating(winRate);

          console.log(
            `${strategy}: ${(winRate * 100).toFixed(1)}% win rate (${rating})`,
          );
        });

        // Overall balance assessment
        const avgWinRate =
          results.reduce((sum, r) => sum + r.playerWins / r.totalGames, 0) /
          results.length;
        console.log(
...
> gameplayTesting.runAllStrategies(5)      // Quick test of all strategies
      ...
      console.log(
        `🤖 Gameplay testing functions exposed to console. Type 'gameplayTesting.help()' for commands.`,
      );
    }
